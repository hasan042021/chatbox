...
export default function Modal({ open, control }) {
  ...
  const [conversation, setConversaion] = useState(undefined);
  ...
  useEffect(() => {
    if (participant?.length > 0 && participant[0].email !== myEmail) {
      ...
      dispatch(
        ...
        .unwrap()
        .then((data) => setConversaion(data))
        .catch((error) => setResponseError("There was a problem!"));
    }
  }, [participant]);
        ...
        <div className="rounded w-[400px] lg:w-[600px] space-y-8 bg-white p-10 absolute top-1/2 left-1/2 z-20 -translate-x-1/2 -translate-y-1/2">
          ...
          >
            ...
            <div>
              <button
                type="submit"
                className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-violet-600 hover:bg-violet-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-violet-500 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none"
                disabled={
